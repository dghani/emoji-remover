...
#[command(
    long_about = "emoji-remover is a multi-threaded tool that removes emoji characters from text files.\n\nIt respects .gitignore by default and supports various programming language file extensions."
)]
struct Args {
...
fn main() {
    ...
    let extensions: Vec<String> = args
        ...
        .unwrap_or_else(|| {
            vec![
                "rs", "js", "ts", "jsx", "tsx", "py", "java", "c", "cpp", "h", "hpp", "go", "rb",
                "php", "cs", "swift", "kt", "scala", "md", "txt",
            ]
            .into_iter()
            .map(String::from)
            .collect()
...
fn process_file(path: &Path, dry_run: bool, quiet: bool) -> Option<usize> {
    ...
    let cleaned_content = EMOJI_PATTERNS.iter().fold(content.clone(), |acc, emoji| {
